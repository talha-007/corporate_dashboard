...
import { fetchCorporateById } from 'src/redux/services/corporateSlice';
...
const EditCorporate = () => {
  ...
  const { corporate } = useSelector((state: RootState) => state.corporates);
  ...
  useEffect(() => {
    if (corporate) {
      ...
      setDepartments(corporate?.departments || []);
    }
  }, [corporate]);
  ...
  const handleSubmit = async () => {
    ...
    if (validations()) {
      try {
        setIsLoading(true);
        const res = await corporateServices.editCorporate(id ?? '', datas);
        console.log('res', res);
        if (res.status === 200) {
        ...
        console.error('Error Editing corporate:', error);
        setIsLoading(false);
      }
    }
  };
