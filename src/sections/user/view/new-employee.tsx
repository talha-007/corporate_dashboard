...
import { fetchAllCorporatesForEmployees } from 'src/redux/services/employeeSlice';
...
const NewEmployee = () => {
  ...
  const { corporates } = useSelector((state: RootState) => state?.employee);
  ...
  const handleSubmit = async () => {
    ...
    if (validations()) {
      try {
        setIsLoading(true);
        const res = await employeeServices.addEmployee(datas);
        console.log('res', res);
        if (res.status === 201) {
